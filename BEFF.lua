...
exploration = Pattern("exploration.png"):similar(0.8)
autobtn = Pattern("auto.png"):similar(0.9)
autoonbtn = Pattern("AutoOn.png"):similar(0.8)
repeatbtn = Pattern("SB_Repeat.png"):similar(0.8)
...
bonus_unit = Pattern("bonus.png"):similar(0.8)
dungeon_clear = Pattern("dungeon_clear.png"):similar(0.8)
unitdatachanged = Pattern("unitdatachanged.png")
lapis = Pattern("Lapis.png"):similar(0.8)
friend = Pattern("friend.png"):similar(0.8)
insufficient_raid_orbs = Pattern("insufficient_raid_orbs.png"):similar(0.8)
raid_orbs_no = Pattern("raid_orbs_no.png"):similar(0.8)
revive = Pattern("lapis_revive.png"):similar(0.8)
giveup = Pattern("giveupnow.png")
esperfilled = Pattern("SB_EsperFilled.png"):similar(0.8)
IsReady = Pattern("SB_MyTurn.png"):similar(0.8)
BackButton = Pattern("backbutton.png"):similar(0.8)
boss = Pattern("bossbattle.png"):similar(0.8)
battle_transition = Pattern("battle_transition.png"):similar(0.8)
...
explorefarm["timber_tracks_exploration"] = Pattern("Timber_Tracks_Exploration.png")
explorefarm["S_maranda_coast_exploration"] = Pattern("maranda_coast_exploration.png")
...
explorePath["timber_tracks_exploration"] = "leaveafterboss|bosscheck,38|right,1000|right,1|down,1000|battleex,lr,9000,14,2000,left|up,1500|right,2000|left,1|up,1|up,1|battleex,lr,18800,29,3000,right|left,10|up,1000|left,1000|down,1000|left,2500|right,4|down,1|left,2000|left,7000|down,1|left,7000|left,7|up,1000|left,1000|up,1000|left,2|up,1000|down,1000|right,3|down,1500|left,6000|left,7000|left,7000|left,7000|right,1000|right,13|up,1000|down,2|left,5000|left,7000|left,7000"
explorePath["S_maranda_coast_exploration"] = "bosscheck,35|down,500|findmove|down,8000|up,13|left,5000|down,1000|right,10|up,3500,collect1|left,500|down,5000|up,7|battle,lr,4800,left|left,10000|up,4000|left,4000|up,2000,collect2|down,2000|up,2|left,3000|left,4000,zone2|findmove|down,500,collect3|right,4000|left,2|down,3000|left,500|down,3000|left,1000|up,1000|left,4000|right,2|up,2000|left,15|down,3000|left,1500|up,2000|left,1000,collect4|right,2000|up,3000|battle,lr,13000,left|left,8000|up,3000|right,1500|up,2000|right,1500|up,1000|right,2000|down,500,collect5|left,2000|down,1500|left,3000|right,3|up,6000"
...
function exploreBattle()
	...
	for i=0,300000 do
		...
		elseif (use_esper_battle and existsL(esperfilled,0)) then
			...
			auto_pressed = 0
		elseif (exists(IsReady,lagx/3)) then
			endTurn()
		end
		usePreviousSnap(false)
...
function skillCast(castdelay)
	...
	--Iterates through the units to prepare tap locations
 	for n=1, (numunits) do
		...
		--
		--	table.remove(sortedDelayKeys[n])
		--end
	end

	--Array that will contain actions and wait times for clicking units
 	local castorder = {}
	...
	if (debug_mode) then runlog("Skillcast took "..(t:check()*60).." frames to complete.") end

	if (not farmloc == "chain_helper") then
		...
		if ((existsL(menuinbattle,lagx*0.75) and not sb_reg:exists(IsReady,lagx*0.75)) or existsL(BackButton,lagx*0.75)) then
			endTurn("autobtn")
		end
		...
		setStopMessage("Performed all taps in "..(t:check()*60).." frames.")
	end
end
...
function smartBattle_companion(skillmp, damage)
	...
	elseif (not sb_reg:exists(IsReady,lagx*0.75)) then
		return false
	end
...
function defineSBreg()
	...
	if (bottom_reg:exists(IsReady,lagx/3)) then
		...
		for i,u in ipairs(findUnit) do
			...
			if (debug_mode or farmloc == "chain_helper") then sb_regunit[i]:highlight(0.2) end
		...
		local foundRightUnits = #findUnit
		...
		for i,u in ipairs(findUnit) do
			...
			if (debug_mode or farmloc == "chain_helper") then sb_regunit[i+foundUnit]:highlight(0.2) end
			if (rX > u:getX()) then rX = u:getX() end
			if (rY > u:getY()) then rY = u:getY() end
			if (rX2 < (u:getX() + u:getW())) then rX2 = u:getX()+u:getW() end
			if (rY2 < (u:getY() + u:getH())) then rY2 = u:getY()+u:getH() end
		...
		if (debug_mode or farmloc == "chain_helper") then sb_reg:highlight(0.2) end

		if (foundRightUnits == 3) then
			companion_used = true
		end

		--WIP stuff to handle define smartbattle region with less than a full party
--[[
		if (foundLeftUnits == 3 and foundRightUnits >= 1) then
			--This is good, we have a nice square sb_reg
		elseif (foundLeftUnits >= 1 and foundRightUnits == 3) then
			--This is good, we have a nice square sb_reg
		elseif (sb_reg:getY()+sb_reg:getH()+sb_regunit[1]:getH() > screen.getH()) then
			--Bottom of sb_reg is good
		elseif(sb_reg:getH() < (sb_regunit[1]:getH()*2.5) or sb_reg:getW() < (sb_regunit[1]:getW()*1.5)) then
			--this is bad
		end
--]]
	end
...
function smartBattle()
	...
	while(true) do
		...
		if(sb_reg:exists(IsReady,0)) then
			usePreviousSnap(false)			if(state == 0) then		-- 0-9 are states for first smart battle handling
				wait(0.1)
...
function smartBattle_arena()
	...
	while(true) do
		...
		if(exists(IsReady,0) and #sb_regunit > 0) then
			usePreviousSnap(false)
			if(state == 0) then -- 0-9 are states for first smart battle handling
				...
				if (sb_reg == nil or #sb_regunit < 5) then defineSBreg() end
				if (#sb_regunit > 0) then smartBattle_choose(arena_skilluse, arena_skillmp) end
--				state = 1
				state = 99
				endTurn("autobtn")				
			elseif(state == 1) then
...
function chainhelpermenu(chaintable)
	...
	dialogInit()
	addTextView("THIS FUNCTION IS CURRENTLY IN TESTING. PLEASE REPORT ANY ISSUES.")
	newRow()
	addTextView("CURRENTLY ONLY SUPPORTS A FULL TEAM.")
	addTextView("")
	newRow()
	...
	addTextView("")
	newRow()
	addTextView("\tUnit 1 Ability Frame Delay: ")
	addEditNumber("chain1",0)
	newRow()
	addTextView("")
	newRow()
	...
	addTextView("")
	newRow()
	dialogShow("Chain Helper")

	chaintable[1] = _G["chain1"]
	chaintable[2] = _G["chain2"]
	chaintable[3] = _G["chain3"]
	chaintable[4] = _G["chain4"]
	chaintable[5] = _G["chain5"]
	chaintable[6] = _G["chain6"]
...
if(script_function == 1) then
	...
	addRadioButton("Custom Skill selection - BREAKS IF STATUS EFFECT OR KO OCCURS ON TURN 1. WORKING ON A SOLUTION.", 3)
...
elseif(script_function == 5) then
	...
	chainhelpermenu(chaintable)
	skillCast(chaintable)
...
while true do
	if(farmloc == "chain_helper") then
		while(sb_reg:exists(IsReady,lagx)) do
			wait(lagx)
		end
		scriptExit()
	end
